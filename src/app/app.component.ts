...
export class AppComponent implements OnInit {
  ...
  constructor(readonly _title: Title, readonly platformLocation: PlatformLocation) {
  }
  ...
  ngOnInit(): void {
    ...
    for (let i = 0; i < 9; i++) {
      ...
      cell.style.background = `url('${this.platformLocation.getBaseHrefFromDOM()}assets/scratch cover.png')`;
      ...
      cell.addEventListener('mousemove', ev => {
        if (clicked) {
          ...
          bubble.style.background = `url('${this.platformLocation.getBaseHrefFromDOM()}assets/${cell_value}.png')`;
      ...
      cell.addEventListener("touchmove", ev => {
        ev.preventDefault()
        const bubble = document.createElement('span');
        bubble.style.left = `${ev.touches[0].clientX - ((this.size / 3)) * 4 / 100}px`
        bubble.style.top = `${ev.touches[0].clientY - ((this.size / 3)) * 4 / 100}px`
        bubble.style.display = "block";
        bubble.style.position = "absolute";
        bubble.style.width = "4%";
        bubble.style.height = "4%";
